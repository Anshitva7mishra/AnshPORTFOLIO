...
export default function SpecialInvestigation() {
  ...
  return (
    <section id="special-investigation" className="my-8 bg-ink/5 text-ink p-4 md:p-6 border-y border-border-custom relative overflow-hidden">
      {/* Background print halftone effect approximation */}
      <div className="absolute inset-0 opacity-10 pointer-events-none bg-[radial-gradient(#111111_1px,transparent_1px)] [background-size:16px_16px]" />

      <div className="relative z-10">
        <div className="border-b border-border-custom/50 pb-2 mb-4">
        ...
        <p className="font-body text-sm md:text-base italic text-ink/80 max-w-3xl mx-auto text-center mb-8">
        ...
        <div className="text-center font-mono text-xs text-ink/60 mb-8">
        ...
        {isLoading ? (
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 my-6">
            {Array.from({ length: 3 }).map((_, idx) => (
              <div 
                key={idx}
                className="border border-border-custom/50 p-4 md:p-5 bg-paper/80 backdrop-blur-sm flex flex-col justify-between animate-pulse select-none"
              >
                <div>
                  <div className="flex justify-between items-center border-b border-border-custom/30 pb-2 mb-4">
                ...
                <div className="border-t border-border-custom/20 pt-4 mt-6 flex justify-between">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 my-6">
            {caseFiles.map((cFile, idx) => (
              <motion.div 
                ...
                className="border border-border-custom/50 p-4 md:p-5 bg-paper/80 backdrop-blur-sm flex flex-col justify-between"
              >
                <div>
                  <div className="flex justify-between items-center border-b border-border-custom/30 pb-2 mb-4">
                  ...
                  <h3 className="font-headline font-black text-lg uppercase tracking-tight text-ink mb-2">
                    {cFile.title}
                  </h3>

                  <div className="font-mono text-[9px] text-ink/60 uppercase tracking-widest mb-4">
                    {cFile.meta}
                  </div>

                  <p className="font-body text-xs leading-relaxed text-ink/80 text-left md:text-justify">
                    {cFile.body}
                  </p>
                </div>

                <div className="border-t border-border-custom/20 pt-4 mt-6 font-mono text-[9px] text-ink/60 uppercase flex justify-between">
